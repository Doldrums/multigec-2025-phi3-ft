...
def process_corpus_files(orig_folder, sub_folder, corpus_name, split, copy_sub1=False):
    ...
    orig_files = [f for f in os.listdir(orig_folder) if f.startswith(
        corpus_name) and f.endswith(f"orig-{split}.md")]
    ...
    if not orig_files:
        ...
        sub_files = sorted([f for f in os.listdir(sub_folder) if f.startswith(
            corpus_name) and f.endswith(f"-{split}.md")])  # could be hypo- or baseline-

        # If no hypo files exist, skip this corpus
        if not sub_files:
            print(
                f"Skipping corpus '{corpus_name}' as no submission files were found in {sub_folder}")
            return
        ...
        for sub_file in sub_files:
            ...
            for essay_id, txt in sub_dict.items():
                ...
                if essay_id in corpus_dict:
                    print(
                        f"Found essay {essay_id} and adding a submission essay for it")
        ...
        for essay_id in corpus_dict:
            if len(corpus_dict[essay_id]["subs"]) == 0:
                print(
                    f"Did not find a corrected version of essay {essay_id} so inserting an exclamation mark for it")
...
def split_to_parfiles_all(orig_folder, sub_folder, split, copy_sub1=False):
    ...
    for corpus_name in corpus_names:
        ...
        process_corpus_files(orig_folder, sub_folder,
                             corpus_name, split, copy_sub1)
